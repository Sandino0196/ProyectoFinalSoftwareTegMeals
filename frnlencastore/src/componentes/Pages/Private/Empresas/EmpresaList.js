...
export default class Empresas extends Component {
 ...
 loadMore(page){
   ...
   const uri = `/api/empresa/empresa/${page}/${items}`;
   saxios.get(uri)
     .then(
       ({data})=>{
         console.log(data);
    ...
    (item)=>{
      return (
        <div className='listItem' key={item._id}>
          <span>{item.id} {item.userCompleteName}</span>
          <span className='updateListItem'>
            <Link to={`/empresas/${item._id}`}>
              <IoIosInformationCircleOutline size='2em'/>
            </Link>
            <Link to={`/empresas/img/${item._id}`}>
              <IoIosImage size='2em' />
            </Link>
          </span>
        </div>);
  ...
  return (
    <Page pageTitle="Empresas" auth={this.props.auth}>
